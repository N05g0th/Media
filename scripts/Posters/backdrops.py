...
TMDB_IMAGE_BASE_URL = "https://image.tmdb.org/t/p/w200"  # tamaño del backdrop
...
def buscar_backdrop(titulo):
    """Busca la película en TMDB y devuelve la URL completa de su backdrop."""
    try:
        resp = requests.get(
            TMDB_SEARCH_URL,
        ...
        backdrop_path = resultados[0].get("backdrop_path")
        if not backdrop_path:
            return None

        return f"{TMDB_IMAGE_BASE_URL}{backdrop_path}"
    except requests.RequestException as e:
        print(f"  ! Error buscando '{titulo}': {e}")
        return None


def construir_extinf(info_original, backdrop_url):
    """Inserta o reemplaza el atributo tvg-logo en la línea EXTINF."""
    if not backdrop_url:
        return info_original  # deja la línea sin cambios si no se encontró backdrop

    if 'tvg-logo="' in info_original:
        return re.sub(r'tvg-logo="[^"]*"', f'tvg-logo="{backdrop_url}"', info_original)
    ...
    if match:
        return f'{match.group(1)} tvg-logo="{backdrop_url}"{match.group(2)}'
...
def main():
    ...
    for idx, entrada in enumerate(entradas, start=1):
        ...
        backdrop_url = buscar_backdrop(titulo_limpio)
        print(f"    -> Backdrop: {backdrop_url if backdrop_url else 'No encontrado'}")

        nueva_info = construir_extinf(entrada["info"], backdrop_url)
