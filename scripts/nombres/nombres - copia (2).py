...
def procesar_m3u(archivo_origen, archivo_resultado, archivo_encontrados):
    try:
        ...
        # Manejo de la cabecera estándar #EXTM3U
        ...
        for i in range(inicio, len(lineas)):
            ...
            if linea.startswith('#EXTINF:'):
                ...
                if len(partes) == 2:
                    ...
                    # Verificar si la siguiente línea es la URL del canal para no perder el enlace
                    ...
                    if info:
                        ...
                        # Guardamos en ambas listas (Completa y Solo Encontrados)
                        ...
                        print(" -> No encontrado")
                        # Si no se encuentra, solo se guarda en la lista general (resultado.m3u)
                ...
                time.sleep(0.25)  # Pausa de seguridad para la API
            
            # Las líneas de URL se manejan de forma emparejada arriba, por lo que las saltamos aquí
            elif i > 0 and lineas[i-1].startswith('#EXTINF:'):
                continue
            else:
                # Cualquier otra línea decorativa o comentario va a la lista general
                if i != 0: 
                    lineas_resultado.append(linea)

        # Guardar archivo 1: Lista completa procesada
        with open(archivo_resultado, 'w', encoding='utf-8') as dest_res:
            dest_res.writelines(lineas_resultado)
            
        # Guardar archivo 2: Lista limpia (Solo los canales exitosos)
        with open(archivo_encontrados, 'w', encoding='utf-8') as dest_enc:
            dest_enc.writelines(lineas_encontrados)
            
        print(f"\n¡Éxito! Archivos guardados correctamente:")
...
if __name__ == "__main__":
    # Ejecuta el procesador generando ambos archivos m3u de salida
